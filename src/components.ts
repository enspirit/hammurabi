...
export interface ComponentCollection { [key: string]: Component }

// find components that are part of the base architecture (= meant to be running in production envs)
export const getComponents = async (composeFile = 'docker-compose.base.yml'): Promise<ComponentCollection> => {
  ...
  const dockerCompose: DockerCompose = yamlParse(await Deno.readTextFile(composeFile)) as DockerCompose;
